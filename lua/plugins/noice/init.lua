...
local function get_config(has_neovide)
	---@diagnostic disable: undefined-doc-name
	return {
		cmdline = {
			enabled = not has_neovide, -- enables the Noice cmdline UI
...
function M.config()
	...
	vim.g.neovide = false
	if has_neovide ~= false then
		config = get_config(has_neovide)
	end

	require('noice').setup(config)
end

function M.pause(should_pause)
	config.messages.enabled = not should_pause
	require('noice').setup(config)
end

-- Nmap('<localleader>n', { enable, true })
-- Nmap('<localleader>N', { enable, false })
