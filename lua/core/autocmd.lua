...
Augroup('Neorg', {
	...
	-- 	-- [[0r !echo "%:t:r"]]
	-- ),
	Autocmd('BufRead', '*/journal/[0-9]*.norg', function(data)
		-- string.split()
		-- local relative = string.match(data.file, '*/journal/(%d*)/(%d*).norg$')
		local filtered = string.match(data.file, 'journal/(.*).norg')
		-- local relative = string.match(data.file, '*/neorg/journal/(.*).norg$')
		print(filtered)
		print(' ')
		vim.cmd([[0r !echo "_*$(date -d '%:t:r' + '\%A, \%b \%d')*_\n"]])
